...
class Puzzle
  ...
  def make_a_guess
    ...
    count_unsolved_cells = {
      r: @s.map { |row| row.count { |cell| cell.class == Array } },
      c: @s.transpose.map { |row| row.count { |cell| cell.class == Array } },
    }
    
    least_unsolved_cells = {
      r: count_unsolved_cells[:r].each_with_index.select { |count, index|
        count > 0
      }.min[1],
      c: count_unsolved_cells[:c].each_with_index.select { |count, index|
        count > 0
      }.min[1]
    }

    r_or_c = least_unsolved_cells[:c] < least_unsolved_cells[:r] ? :c : :r

    if r_or_c == :r 
      j = least_unsolved_cells[:r]
      i = @s[j].each_with_index.find { |cell, k| cell.class == Array }[1]
    else
      i = least_unsolved_cells[:c]
      j = @s.transpose[i].each_with_index.find { |cell, k| cell.class == Array }[1]
    end
    # p least_unsolved_cells
