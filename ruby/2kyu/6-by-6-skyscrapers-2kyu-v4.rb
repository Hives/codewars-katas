...
class Puzzle
  ...
  #   self.method_printy
  #   @count[:rule_3] += 1
  #   # if clue = 2 and there's a 4 in the row/col then the cell closest to the
  #   # clue must be higher than any other cell between it and the 4
    
  #   [:r, :c].each do |r_or_c|
  #     @s = @s.transpose if r_or_c == :c
  #     @c[r_or_c].each_with_index do |clues, i|
  #       [:f, :b].each do |f_or_b|
  #         if clues[f_or_b] == 2
  #           row = @s[i]
  #           row.reverse! if f_or_b == :b
