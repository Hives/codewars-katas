...
class Puzzle
  ...
  def initialize(clues)
    ...
    @dim = clues.size/4
    @clues = {
      r: Array.new(@dim){|i| { f: clues[-(i + 1)], b: clues[@dim + i] } },
      c: Array.new(@dim){|i| { f: clues[i], b: clues[-(i + 1 + @dim)] } }
    }
    # @sol = Array.new(@dim) { Array.new(@dim) { (1..@dim).to_a } }.flatten(1)
    @sol = (1..@dim*@dim).to_a
    @guesses = []
    @failed = false
    ...
    #           row[n] -= (@d+2+n-@c[r_or_c][i][f_or_b]..@d).mto_a
    #         end
    #       end
    #       row.reverse! if f_or_b == :b
    #     end
  ...
  def get_row(j)
    return Array.new(@dim) { |i| @sol[j*@dim + i] }
  end

  def set_row(j, row)
    row.each_with_index { |content, i| set_cell(i, j, content) }
    return row
  end

  def get_col(i)
    return Array.new(@dim) { |j| @sol[j*@dim + i] }
  end

  def set_col(i, col)
    col.each_with_index { |content, j| set_cell(i, j, content) }
    return col
  end

  def set_cell(i, j, content)
    @sol[j*@dim + i] = content
  end
  ...
  def solve
    ...
    set_row(2, [1,1,1,1,1,1])
    set_col(4, [5,5,5,5,5,5])
