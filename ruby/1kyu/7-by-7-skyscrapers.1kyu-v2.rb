...
class Puzzle
  ...
  def no_duplicates_rule
    ...
    no_change_at_all = true
    loop do
      no_change_this_iteration = true
      [:r, :c].each do |r_or_c|
        (0..@d-1).each do |i|
          row = get_r_or_c(r_or_c, i)
          # puts "#{r_or_c} #{i}: #{row}"
          ...
          if solved.size > 0 && solved.size < @d
            row.map! do |cell|
              if cell.is_a? Array
                ...
                if cell.size != original_size
                  no_change_at_all = false 
                  no_change_this_iteration = false
                end
      ...
      break if no_change_this_iteration
    end
    printy
    return "no change" if no_change_at_all
    return "changed"
  end

  def no_other_option_rule
    # if a number can only go in one cell of a row or column then it must go there
    method_printy
    count_method_use
    printy
  ...
  def filter_solutions_using_clues
    method_printy
    count_method_use
    
    [:r, :c].each do |r_or_c|
      (0..@d-1).each do |i|
        filtered_row = filter_row_or_c_using_clues(r_or_c, i)
        if filtered_row == false
          return false
        end
        set_r_or_c(r_or_c, i, filtered_row)
      end
    end
  ...
  def filter_row_or_c_using_clues(r_or_c, i)
    row = get_r_or_c(r_or_c, i)
    row_possibilities = []

    # get list of all permutations of current possibilities
    loop do
      poss = row.shift
      poss = [poss] if !poss.is_a? Array
      if row_possibilities.size == 0
        poss.each { |p| row_possibilities += [[p]] }
      else
        partial_possibilities = []
        row_possibilities.each do |row_part_poss|
          poss.each do |n|
            partial_possibilities += [row_part_poss + [n]] if !row_part_poss.include? n
          end
        end
        row_possibilities = Marshal.load(Marshal.dump(partial_possibilities))
      end
      break if row.size == 0
    end

    row_possibilities.delete_if { |poss| !check_row_or_col_2(poss, @c[r_or_c][i]) }
    row_possibilities = row_possibilities.transpose.map do |poss|
      output = poss.uniq
      output = output[0] if output.size == 1
      output
    end
    
    return false if row_possibilities.include? nil
    return false if row_possibilities.size != @d
    row_possibilities
  end
  ...
  def roolz
    ...
    return "invalid" if no_duplicates_rule == false
    return "invalid" if no_other_option_rule == false
    return "invalid" if filter_solutions_using_clues == false
  ...
  def check_row_or_col(r_or_c, i)
    ...
    row = get_r_or_c(r_or_c, i)
    return check_row_or_col_2(row, @c[r_or_c][i])
    # p true
  end

  def check_row_or_col_2(row, clues)
...
clues1 = [7,0,0,0,2,2,3,
          0,0,3,0,0,0,0,
          3,0,3,0,0,5,0,
          0,0,0,0,5,0,4]

clues2 = [0,2,3,0,2,0,0,
          5,0,4,5,0,4,0,
          0,4,2,0,0,0,6,
          5,2,2,2,2,4,1]

[clues1, clues2].each do |clues|
